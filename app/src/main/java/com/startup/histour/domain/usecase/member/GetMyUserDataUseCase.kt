...
) : BaseUseCase<UserInfoModel, Unit>(
    coroutineScope = coroutineScope,
    preExecutionContext = preExecutionContext,
    postExecutionContext = postExecutionContext
) {
    override suspend fun buildUseCase(params: Unit): Flow<UserInfoModel> =
        memberRepository.getUserInfo().map {
            it.toUserInfoModel()
        }.onEach {
            userInfoDataStoreProvider.setUserInfo(
                it.userName,
                it.profileImageUrl,
                it.character.toCharacterInfo()
            )
        }
}